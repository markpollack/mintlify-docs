...
<Note>
**Explicit always wins.** If you set `spring.ai.agents.codex.skip-git-check=true` alongside `spring.ai.agents.mode=strict`, the explicit property wins. See [Defaults Philosophy](/agent-client/defaults-philosophy) for the full rationale.
</Note>
...
See [Defaults Philosophy](/agent-client/defaults-philosophy) for detailed mode-vs-property interaction examples.
...
<CardGroup cols={3}>
  <Card title="Claude" icon="terminal" href="/agent-client/claude-reference">
    18 properties — `spring.ai.agents.claude-code.*`
  </Card>
  <Card title="Codex" icon="code" href="/agent-client/codex-reference">
    6 properties — `spring.ai.agents.codex.*`
  </Card>
  <Card title="Gemini" icon="sparkles" href="/agent-client/gemini-reference">
